...
size = 16071 # Determined from data. See section 3.2.
...
questions2 = []
questions =[]
with open('test100.json', encoding='utf-8') as jsondata:
...
for i in range(len(questions1)):
    if i % 2000 == 0:print("Processing the pair %s ",i," Questions")

    this_query = questions1[i]

    l_Q = np.zeros( (1, len(this_query) , WORD_DEPTH) )
    ...
    l_P = np.zeros((1, len(this_query), WORD_DEPTH))
    for n in range(len(this_query)):
        this_query_split = this_query[n:(n + 3)]
...
Nega_vec = np.zeros( (1, len(Nega) , WORD_DEPTH) )
...
print("This predict accuracy is :", accuracy/test_number )
